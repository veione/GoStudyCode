...
func (ins *NsqInstance) newProducerPool(poolSize int) {
	...
	nsqdLen := len(nsqds)
	if nsqdLen <= 0 {
		logger.Errorf("查询nsqd节点数量为0 无法创建生产者 nsqds:%v", nsqds)
		return
	}
	logger.Debugf("**********创建producter池************：")
	// poolSize 表示向每个nsq实例上连接的数量
	for i := 0; i < poolSize; i++ {
		for _, addr := range nsqds {
			...
			logger.Debugf("nsqd: %v", addr)
		}
	}
}

// NsqNodeData nsqd节点信息
...
func (ins *NsqInstance) newConsumer(topic, channl string, handler nsq.Handler) *nsq.Consumer {
	...
	if err != nil {
		logger.Errorf("NewConsumer err:%v", err)
		return nil
	}
